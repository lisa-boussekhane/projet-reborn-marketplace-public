...
async getOneShop(req, res) {
    try {
          ...
          res.status(500).json({ message: 'Failed to retrieve shop details', error: error.message });
        }
      },
...
async updateShop(req, res) {
    ...
    try {
          ...
          const shop = await Shop.findOne({ where: { id: shopId, user_id: userId } });
          if (!shop) {
            return res.status(404).json({ error: 'Shop not found or does not belong to the user' });
          }
